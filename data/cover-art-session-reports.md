...
**Ep229 (2026-07-25, second rejection — wrong clause weighted as dominant):** The redo above overcorrected: every concept in the second batch was built entirely around "Contender" (bottle-height measuring, lineup-fitting, spin-inspection) with zero connection to "Summer Sipper" — the episode's actual dominant content (the whole cold-open segment, 8 named competing wines) and something Joe had explicitly told Claude to focus on twice earlier in the same session. Joe: "This happens every week." Root cause: the thesis/title-alignment gate had no mechanism for weighting multiple clauses of a title by actual episode content — Claude defaulted to whichever clause was easiest to storyboard (a literal comparison mechanic) rather than the one with the most real content behind it. Fixed permanently via amendments to HR-53/HR-54 in `docs/house-rules.md` (list every title clause, weight each by episode content and standing host instructions, anchor the thesis in the highest-weighted clause) and matching updates to `.claude/commands/generate-cover-art.md` and `.claude/commands/review-titles.md`.
...
**Joe's correction (2026-07-25, third round):** Joe rejected the recommendation (Concept B) again, this time for the opposite problem: "You took one small aspect of the episode and made a big deal of it, and it is not really following the concept of the title." B's specificity (a real, three-line running joke about sparkling wine) came at the cost of thematic weight — it's a minor aside, not the sensory core of "summer sipper" (heat, refreshment, cold-drink relief), which A and C both stage directly. Joe selected between A and C and confirmed **Concept A**. Codified as an addition to HR-53 in house-rules.md: specificity/portability is a floor, not a tiebreaker that overrides how directly a concept visualizes the dominant clause's actual content — a narrowly-specific concept tied to a minor moment can still lose to a broader concept that more viscerally captures the theme.
