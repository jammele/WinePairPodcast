...
**SEO/AEO + social:** `outputs/episodes/ep227-gigondas.md` — Key Questions (7), FAQ (7 pairs), and 10 Bluesky posts for the two-wine Gigondas review (2023 Notre Dame des Pallières Gigondas Les Mourres, 2023 Crous St Martin Gigondas Les Espaliers). Schema Markup not generated, not requested this session. Validator passed clean on first run.

**FAQ correction (same session):** First-pass Key Questions/FAQ shipped with 2 of 7 questions specific to the exact bottles reviewed, with one leading the list. Joe flagged this as a repeat of prior feedback and asked for the underlying process to be fixed, not just the output. Root cause: `docs/faq-intent-model.md` Criterion 4 (Specificity) scored SKU-narrow questions higher than broad grape/region/style questions, backwards for a show whose growth mechanism is new-listener discovery via broad search. Re-researched FAQPage/AEO best practices (Google's own guidance ties specificity to page topic, not individual SKU; quality/reach beats a fixed question count) and corrected the intent model: added a guiding principle, made the priority ranking conditional on whether the reviewed product has independent search demand, fixed Criterion 4, and added a hard cap of 1 SKU-specific question per standard review episode (not leading). Regenerated Ep227's Key Questions/FAQ through the corrected process; final list leads with "What does Gigondas taste like?" with exactly 1 bottle-specific question, placed last. Full corrected evidence ledger and scoring at `outputs/episodes/faq-audits/ep227-faq-audit.md` (overwrote the prior version). Validator passed clean.
