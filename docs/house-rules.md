...
**Session-specific instructions are evaluated and reported separately from HR-17.** If Joe gives an additional instruction for the current title-generation session (e.g., asking that some other term also appear early, or any other one-off constraint not codified above), it must never be folded into or reported as the HR-17 PASS/FAIL — HR-17 is decided only by its own text above. Track and report a session instruction as its own labeled line, `Session constraint: PASS` or `Session constraint: NEEDS REVISION`, alongside the HR-17 result (a title can be `HR-17: PASS` and `Session constraint: NEEDS REVISION` at the same time). A session constraint is still binding for that session: a title marked `NEEDS REVISION` must be revised before it is presented to Joe, even though it is not a standing house rule.
...
**Before presenting, verify the final displayed list's order actually matches its claimed ordering method** — e.g., if the list is labeled as being in Clickability order, confirm the printed scores actually descend. If they don't, either fix the order or explicitly state the different ordering principle actually used; never present a mislabeled order as sorted. This check applies to the final list shown to Joe, not only an intermediate candidate list. Clickability remains an internal comparative heuristic regardless of order — never measured performance.

**AEO reporting must use the current standard in `docs/title-research-reference.md`, not an invented superiority claim.** Do not describe a named-entity placement, or any other wording choice, as "the strongest positioning" or any other ranking/performance claim the reference file doesn't state. When the reference's current standard is that candidates sharing a named entity show no evidence-based differentiation, report exactly that (or the reference's then-current equivalent wording) rather than a paraphrase implying a measured advantage.

**Any structural description of a title in a report must match the title's actual wording.** Do not label a title that combines a question and a statement (or any other compound structure) using a term that fits only one of its clauses — name the full structure. This is a reporting-accuracy requirement, not a new scoring category.

**When presenting a batch regenerated after Joe rejected a prior batch in full, check that the surviving options are meaningfully distinct** in listener promise, structure, or episode-supported angle — not just wording variants of one validated idea. Do not present near-paraphrases as if they were distinct choices, and do not revive an angle Joe already rejected this session merely to manufacture variety. There is no required minimum number of angle types and no required batch size: if the session's constraints genuinely leave only one strong direction, say so explicitly and present fewer, meaningfully distinct options rather than padding to a fixed count.
...
- whether a small wording change — an article, pronoun, tense, or singular/plural form — materially changes what the title claims relative to the episode's actual content, naming the specific difference when it does. This applies only when the change alters meaning; do not flag routine stylistic variation, and do not generalize this into a universal grammar rule. A meaning-changing wording issue is a qualitative Voice Fit note, not a numeric score or an automatic FAIL — it becomes a hard-rule issue only if the changed meaning itself creates a genuine violation of an existing rule (e.g., turns an accurate claim into a spoiler or an unsupported claim).
...
**HR-31: Bluesky post URLs — at most two podcast-link posts; every other post is a factual/external post.**
Joe's standing direction: no more than two posts in a weekly batch may link to or promote the podcast — this is a ceiling, not a target. A batch may contain zero, one, or two podcast-link posts (linking to thewinepairpodcast.com or the specific episode URL); three or more is a hard validation failure. Podcast-link posts, when used, are conventionally placed first, but that placement is a presentation convention, not a requirement — a post is classified as a podcast-link post or a factual post by its URL, not by its position. Every post that does not link to the podcast is a factual post and must carry a unique external URL — no repeats, no thewinepairpodcast.com. Factual posts make up most of the batch and should not read like ten advertisements for the show. Use the episode script's research links, or other credible external sources, to assign one unique source per factual post.

Additional requirements for factual posts:
- Domain diversity: no more than 2 factual posts may share the same domain (e.g. en.wikipedia.org). If more than 2 would be the same domain, find alternatives from Decanter, Wine Enthusiast, Wine Folly, Wine-Searcher, specialized wine blogs, or news sources.
- Grounding: every factual post must start from a claim or idea actually discussed in the episode, and its linked source must corroborate that specific claim, not just be a page that happens to concern the same wine, grape, producer, or region. A source's existence does not make the drafted wording accurate — compare the post's precise claim against what the source actually says before finalizing.
- Angles are earned, not scheduled: Nerd, story, history, production, geography, and misconception framings are useful only when the episode actually covered that content. There is no required minimum count of any angle, and no angle should be forced onto a post merely to fill a slot.
- A pre-write plan table (see generate-episode-content.md Section 4) must be completed before any post is written. The plan confirms the podcast-link count is 0-2, URL uniqueness, and domain diversity before text is drafted.
- The ten-post total, the 300-character limit (HR-26), and the episode-grounding requirements apply regardless of how many podcast-link posts are used.
...
6. **Every factual post starts from an episode-covered claim, and the source must corroborate that specific claim.** Not a general page about the same wine, grape, producer, or region — the source must actually support the precise statement the post makes. External research must never introduce an unrelated fact merely because it's interesting or happens to concern the same topic.
7. **A source's existence does not make the drafted wording accurate.** Compare the post's precise claim against what the source actually says before finalizing, every time — do not assume a plausible-sounding sentence is supported just because a real URL is attached to it.
8. **Avoid in factual posts:** "we talked about" or "in this episode" framing; repeated ratings/verdict hints; finishing-wine updates; generic promotion of another publication's article or list with no concrete fact attached; vague claims like "the wines argue otherwise"; constructed ad-copy phrasing; unsupported generalizations; presenting an unranked list as a ranking; and misidentifying a retailer, marketplace, or commercial site as an official regulatory or consortium body.
9. **Minimize excessive self-reference outside any podcast-link posts.** Routine posts about which wine the hosts finished, their internal process, or a repeated verdict tease do not belong among the factual posts.
...
- An exact-bottle/SKU question is capped, never reserved: for a standard boutique-bottle review, no more than one final question may be about the exact bottles reviewed, and it earns that slot only when the product, producer, or exact comparison has documented independent audience/discovery relevance (topic-specific owned evidence, or a recognized brand/retailer/product). A distinctive verdict, comparison, or episode-grounded answer does not by itself establish that relevance, and a site-wide or adjacent-category prior does not substitute for topic-specific evidence. See `docs/faq-intent-model.md` §3.
