# Ep225: Costco Kirkland Signature Challenge: Pauillac vs. Saint-Julien Bordeaux!

**Google Doc ID:** 148HBLjDZaWnVsZDJCkcCRVVoxhy82MQCKF5PKSU1gv4
**Generated:** 2026-07-03

---

## KEY QUESTIONS

*(Place at top of show notes - questions only, no answers)*

- What is the difference between Pauillac and Saint-Julien Bordeaux?
- What does the Costco Kirkland Signature Pauillac taste like?
- What does the Costco Kirkland Signature Saint-Julien taste like?
- Is the Kirkland Pauillac worth buying at Costco?
- What food pairs with Pauillac and Saint-Julien Bordeaux?
- Why is Pauillac wine so expensive?
- What is a Bordeaux negotiant and how does it work?

---

## FREQUENTLY ASKED QUESTIONS

**Q. What is the difference between Pauillac and Saint-Julien Bordeaux?**
A. Both are Medoc appellations in Bordeaux, but they differ in style and prestige. Pauillac is home to three of Bordeaux's five First Growth estates: Lafite, Latour, and Mouton Rothschild, and typically blends 70-80% Cabernet Sauvignon. Saint-Julien is the smallest major Medoc appellation, has 11 classified growths, and uses closer to 60% Cab Sav.

**Q. What does the Kirkland Signature Saint-Julien Bordeaux taste like?**
A. Smooth and juicy, with blueberry, blackcurrant, prune, floral notes, spice, and pencil shavings. The nose is very aromatic, with strong plummy and blackberry aromas. It's a 62% Cabernet Sauvignon, 38% Merlot blend with a smooth medium-long finish. It earned a 7/10 rating.

**Q. What does the Kirkland Signature Pauillac taste like?**
A. Juicy and jammy, with dark cherry, plum, blackcurrant, blackberry, cedar, tobacco, pencil lead, and spice. It's a young wine that benefits from extra time in the glass or decanting. It earned an 8/10 rating and professional scores of 91-92 points.

**Q. Is the Costco Kirkland Signature Pauillac worth buying?**
A. Yes. It's priced under $25 at Costco and earned an 8/10 rating along with 91-92 points from professional reviewers. Pauillac wines are rarely available at this price. The wine is young and benefits from more time in the bottle, but it's a genuine value for the appellation.

**Q. What food pairs with Pauillac and Saint-Julien Bordeaux?**
A. Both wines pair well with red meat, steak, steak frites, and filet mignon. The Pauillac also works with cheeseburgers, pork tenderloin, beef stroganoff, and blue cheese sauce. The Saint-Julien pairs well with lasagna, spaghetti and meatballs, and pasta with red sauce. Bold Left Bank Bordeaux needs food with weight and fat.

**Q. Why is Pauillac wine so expensive?**
A. Pauillac is home to three of Bordeaux's five First Growth estates: Lafite, Latour, and Mouton Rothschild. Those estates built an elite reputation for the entire appellation. Most Pauillac bottles cost far more than $25. Finding one at Costco's price range is genuinely uncommon.

**Q. What is a Bordeaux negotiant and how does it work?**
A. A negotiant is a wine merchant that buys grapes or finished wine from multiple chateaus and bottles under its own label. The Kirkland Signature Saint-Julien is bottled by Maison Gineste, a negotiant house founded in 1897 that partners with more than 350 chateaus across Bordeaux.

---

## SCHEMA MARKUP

### Review Schema: 2023 Kirkland Signature Saint-Julien Bordeaux

```json
{
  "@context": "https://schema.org",
  "@type": "Review",
  "name": "2023 Kirkland Signature Saint-Julien Bordeaux Review",
  "itemReviewed": {
    "@type": "Product",
    "name": "2023 Kirkland Signature Saint-Julien Bordeaux",
    "description": "A 62% Cabernet Sauvignon, 38% Merlot Left Bank Bordeaux bottled by negotiant Maison Gineste, available at Costco.",
    "offers": {
      "@type": "Offer",
      "price": "18.49",
      "priceCurrency": "USD",
      "seller": {
        "@type": "Organization",
        "name": "Costco"
      }
    }
  },
  "reviewRating": {
    "@type": "Rating",
    "ratingValue": "7",
    "bestRating": "10",
    "worstRating": "1"
  },
  "reviewBody": "Smooth and juicy with blueberry, blackcurrant, prune, floral notes, spice, and pencil shavings, this 62% Cabernet Sauvignon, 38% Merlot blend from Saint-Julien has a very lifted aromatic nose and a smooth medium-long finish. It pairs well with steak, pasta with red sauce, lasagna, and blue cheese. Rated 7 out of 10.",
  "author": [
    {"@type": "Person", "name": "Joe Mele"},
    {"@type": "Person", "name": "Carmela"}
  ],
  "publisher": {
    "@type": "Organization",
    "name": "The Wine Pair Podcast"
  }
}
```

### Review Schema: 2023 Kirkland Signature Pauillac Bordeaux

```json
{
  "@context": "https://schema.org",
  "@type": "Review",
  "name": "2023 Kirkland Signature Pauillac Bordeaux Review",
  "itemReviewed": {
    "@type": "Product",
    "name": "2023 Kirkland Signature Pauillac Bordeaux",
    "description": "A Cabernet Sauvignon-heavy Left Bank Bordeaux from Pauillac, aged 9 months in French oak, available at Costco.",
    "offers": {
      "@type": "Offer",
      "price": "Under $25",
      "priceCurrency": "USD",
      "seller": {
        "@type": "Organization",
        "name": "Costco"
      }
    }
  },
  "reviewRating": {
    "@type": "Rating",
    "ratingValue": "8",
    "bestRating": "10",
    "worstRating": "1"
  },
  "reviewBody": "Juicy and jammy with dark cherry, plum, blackcurrant, blackberry, cedar, tobacco, and spice, this Pauillac is a young wine aged 9 months in French oak that benefits from extra time in the bottle. It pairs well with steak, filet mignon, blue cheese sauce, beef stroganoff, and pork tenderloin. Rated 8 out of 10 and chosen as the preferred finish wine over the Saint-Julien.",
  "author": [
    {"@type": "Person", "name": "Joe Mele"},
    {"@type": "Person", "name": "Carmela"}
  ],
  "publisher": {
    "@type": "Organization",
    "name": "The Wine Pair Podcast"
  }
}
```

### FAQPage Schema

```json
{
  "@context": "https://schema.org",
  "@type": "FAQPage",
  "mainEntity": [
    {
      "@type": "Question",
      "name": "What is the difference between Pauillac and Saint-Julien Bordeaux?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Both are Medoc appellations in Bordeaux, but they differ in style and prestige. Pauillac is home to three of Bordeaux's five First Growth estates: Lafite, Latour, and Mouton Rothschild, and typically blends 70-80% Cabernet Sauvignon. Saint-Julien is the smallest major Medoc appellation, has 11 classified growths, and uses closer to 60% Cab Sav."
      }
    },
    {
      "@type": "Question",
      "name": "What does the Kirkland Signature Saint-Julien Bordeaux taste like?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Smooth and juicy, with blueberry, blackcurrant, prune, floral notes, spice, and pencil shavings. The nose is very aromatic, with strong plummy and blackberry aromas. It's a 62% Cabernet Sauvignon, 38% Merlot blend with a smooth medium-long finish. It earned a 7/10 rating."
      }
    },
    {
      "@type": "Question",
      "name": "What does the Kirkland Signature Pauillac taste like?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Juicy and jammy, with dark cherry, plum, blackcurrant, blackberry, cedar, tobacco, pencil lead, and spice. It's a young wine that benefits from extra time in the glass or decanting. It earned an 8/10 rating and professional scores of 91-92 points."
      }
    },
    {
      "@type": "Question",
      "name": "Is the Costco Kirkland Signature Pauillac worth buying?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes. It's priced under $25 at Costco and earned an 8/10 rating along with 91-92 points from professional reviewers. Pauillac wines are rarely available at this price. The wine is young and benefits from more time in the bottle, but it's a genuine value for the appellation."
      }
    },
    {
      "@type": "Question",
      "name": "What food pairs with Pauillac and Saint-Julien Bordeaux?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Both wines pair well with red meat, steak, steak frites, and filet mignon. The Pauillac also works with cheeseburgers, pork tenderloin, beef stroganoff, and blue cheese sauce. The Saint-Julien pairs well with lasagna, spaghetti and meatballs, and pasta with red sauce. Bold Left Bank Bordeaux needs food with weight and fat."
      }
    },
    {
      "@type": "Question",
      "name": "Why is Pauillac wine so expensive?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Pauillac is home to three of Bordeaux's five First Growth estates: Lafite, Latour, and Mouton Rothschild. Those estates built an elite reputation for the entire appellation. Most Pauillac bottles cost far more than $25. Finding one at Costco's price range is genuinely uncommon."
      }
    },
    {
      "@type": "Question",
      "name": "What is a Bordeaux negotiant and how does it work?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "A negotiant is a wine merchant that buys grapes or finished wine from multiple chateaus and bottles under its own label. The Kirkland Signature Saint-Julien is bottled by Maison Gineste, a negotiant house founded in 1897 that partners with more than 350 chateaus across Bordeaux."
      }
    }
  ]
}
```

---

### BLUESKY POSTS (10)

### Planning Table

| Post | Angle | URL |
|---|---|---|
| 1 | Teaser: Pauillac back at Costco after 7 years | thewinepairpodcast.com |
| 2 | Teaser: elite Bordeaux for under $25, value story | thewinepairpodcast.com |
| 3 | Teaser: ratings hint (both wines rated) | thewinepairpodcast.com |
| 4 | Nerd angle: blend composition, Left Bank structure | https://www.wine-searcher.com/regions-pauillac |
| 5 | Story angle: 1855 Classification, Napoleon III | https://www.thewinecellarinsider.com/bordeaux-wine-producer-profiles/bordeaux/1855-bordeaux-classification/ |
| 6 | Fact: Pauillac has three First Growths (Lafite/Latour/Mouton) | https://www.cellartours.com/destinations/bordeaux/wine-regions/pauillac |
| 7 | Fact: Saint-Julien is the smallest major Medoc appellation | https://www.bordeaux.com/en/designations/medoc/saint-julien/ |
| 8 | Fact: Bordeaux crisis and vineyard pull-out scheme | https://www.decanter.com/wine-news/bordeaux-vineyard-grubbing-up-scheme-hampered-by-weather-535310/ |
| 9 | Fact: Dutch engineers drained the Medoc in the 1600s | https://en.wikipedia.org/wiki/Pauillac_AOC |
| 10 | Fact: negotiant system, how Kirkland wines are sourced | https://www.thewinecellarinsider.com/wine-topics/negociants-courtiers-en-primeur-bordeaux-wine-system/ |

Domain check: thewinecellarinsider.com appears in posts 5 and 10 (2 posts, at limit). All other domains appear once.
Angles coverage: Post 4 = Nerd angle. Post 5 = Story angle.

---

**1.**

Costco's Kirkland Pauillac hasn't been on shelves for 7 years. Pauillac is home to Lafite, Latour, and Mouton Rothschild. We found one for under $25 and tasted it against the Kirkland Saint-Julien. New episode.

thewinepairpodcast.com

**2.**

Two of Bordeaux's most prestigious Medoc appellations. Both Kirkland Signature wines. Both under $25 at Costco. Pauillac and Saint-Julien in the same tasting. New episode.

thewinepairpodcast.com

**3.**

Honest ratings: Kirkland Saint-Julien 7/10, Kirkland Pauillac 8/10. Both got pro scores of 91-93 points. For under $25 each at Costco, these are worth knowing about. New episode.

thewinepairpodcast.com

**4.**

Left Bank Bordeaux is always a blend: Cab Sav for structure, Merlot for softness, Cab Franc for freshness. Pauillac runs 70-80% Cab Sav. Saint-Julien closer to 60%. That's why they taste different from pure California Cab.

https://www.wine-searcher.com/regions-pauillac

**5.**

Napoleon III requested the 1855 Bordeaux Classification for the Paris World's Fair. Pauillac took 3 of the 5 First Growths. The list is still in use today, mostly unchanged.

https://www.thewinecellarinsider.com/bordeaux-wine-producer-profiles/bordeaux/1855-bordeaux-classification/

**6.**

Pauillac is the only Medoc appellation with three First Growth estates: Lafite, Latour, and Mouton Rothschild. Three of the most famous wine names on earth, all from the same small patch of riverbank.

https://www.cellartours.com/destinations/bordeaux/wine-regions/pauillac

**7.**

Saint-Julien is the smallest major Medoc appellation but packs in 11 classified growths. No First Growths, but it's known for "super second" estates. A small appellation with serious prestige.

https://www.bordeaux.com/en/designations/medoc/saint-julien/

**8.**

Bordeaux is in crisis. Thousands of acres of vines are being pulled up in a government-funded scheme as demand falls and costs rise. More than a trend. A structural shift.

https://www.decanter.com/wine-news/bordeaux-vineyard-grubbing-up-scheme-hampered-by-weather-535310/

**9.**

The Medoc was marshland until the 1600s when Dutch hydraulic engineers were hired to drain it. The wine region that gave us Pauillac and Saint-Julien was literally built on reclaimed swamp.

https://en.wikipedia.org/wiki/Pauillac_AOC

**10.**

A Bordeaux negotiant buys from multiple chateaus and bottles under its own label. Maison Gineste, founded in 1897, has 350+ partner chateaus. That's how the Kirkland Saint-Julien is made.

https://www.thewinecellarinsider.com/wine-topics/negociants-courtiers-en-primeur-bordeaux-wine-system/
