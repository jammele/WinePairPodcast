...
**How to use:**
- **PRIMARY (recommended):** Open a new ChatGPT chat. Upload the most recent approved cover art image. Paste: *"Use this as the style reference, but create a new scene. Do not copy the exact pose."* followed by the **(PRIMARY) Scene description** for whichever concept you choose. The image is the style guide — this approach is more reliable than text alone.
- **FALLBACK (no reference image available):** Paste the **(FALLBACK) Full ChatGPT prompt** directly into ChatGPT with no image attachment.
...
*The joke:* Joe and Carmela in full summer relaxation mode, holding chilled red wine glasses dripping with condensation, radiating the smug energy of people who have figured something out that nobody else has.
